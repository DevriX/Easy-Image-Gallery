...
jQuery(window).on("load", function() {
	...
	if ( jQuery('#eig-gallery').length > 0 ) {
		var element = document.getElementById('eig-gallery');
   		var drake = dragula({ containers: [element] }).on('drop', (el, target) => {
            window.test = 1;
    ...
    var MutationObserver = window.MutationObserver || window.WebKitMutationObserver,
        domChangeCallback = function (mutations) {
        	...
        	if (mutations[0].target.id == 'eig-gallery' ) {
    			...
    			element.setAttribute('id', 'eig-gallery-inserted');
        		return false;
        	}

        },
        observer = new MutationObserver(domChangeCallback), // nowa instancja MutationObserver
        domElement = document.querySelector('#editor'); // jakiś element DOM którego zmiany wartości atrybutów będziemy obserwować, np. '<div class=".element-to-observe" data-type="default"></div>'
